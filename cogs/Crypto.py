...
class Crypto(commands.Cog):
    ...
    async def info(
        ...
        embed.set_image(url="https://cdn.discordapp.com/attachments/727659575196647425/820054574068531200/cryptoend.gif")
    ...
    async def buy(
        ...
        if ctx.author.id in self.users_in_transaction:
            return await ctx.respond("You are currently in a transaction, please wait.", ephemeral=True)
        ...
        if amount*crypto.price > user.balance:
            return await ctx.respond("You do not have enough money to buy this crypto.", ephemeral=True)
        
        self.users_in_transaction.append(ctx.author.id)
        
        user.balance -= amount * crypto.price
        user.crypto_balance[tag.upper()] = user.crypto_balance.get(tag.upper(), 0) + amount
        await user.save()
        
        self.users_in_transaction.remove(ctx.author.id)
        
        await ctx.respond("You have bought `{}` of `{}` for `{}$`.".format(amount, crypto.tag, amount * crypto.price))
    ...
    async def sell(
        ...
        if amount > user.crypto_balance.get(tag.upper(), 0):
            return await ctx.respond(f"You do not have enough `{tag.upper()}` to sell.", ephemeral=True)
        ...
        await ctx.respond("You have sold `{}` of `{}` for `{}$`.".format(amount, crypto.tag, amount * crypto.price))
        
        
    @crypto.command(name="graph", description="Grapf of crypto in given period of time.")
    async def graph(
        self, ctx: discord.ApplicationContext, 
        tag: discord.Option(str, description="The crypto tag."),
        period: discord.Option(str, description="The period to check the price for.", choices=["hour", "day", "week"])
    ):
        crypto = await CryptoModel.get_or_none(tag=tag.upper())
        if crypto is None:
            return await ctx.respond("Invalid crypto tag.", ephemeral=True)
        ...
        prices = await CryptoHistoryModel.filter(
                crypto_id=crypto.id, created_at__gte=discord.utils.utcnow() - datetime.timedelta(**{period+"s": 1})
        ...
        fig, ax = plt.subplots()
        df = pd.DataFrame({crypto.tag: [price.price for price in prices]}, index=[price.created_at for price in prices])
        
        minimal_value = int(df[[crypto.tag]].min()) - 1
        maximal_value = int(df[[crypto.tag]].max()) + 1
        
        df.plot(color=colors, ax=ax, xticks=[], yticks=[minimal_value, maximal_value], title=title)
        n_shades = 10
        diff_linewidth = 1.05
        ...
        ax.set_ylim(minimal_value-(maximal_value-minimal_value)*0.1, maximal_value+(maximal_value-minimal_value)*0.1)
